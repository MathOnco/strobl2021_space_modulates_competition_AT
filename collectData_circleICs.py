...
# ================================ Script setup ==========================================
# For n0-fR relationship
initialSizeList = [0.35, 0.5, 0.65]
rFracList = [0.1, .01, .001]
costList = [0.]
turnoverList = [0.,0.3]

# For cR-dT relationship
# initialSizeList = [0.25,0.75]
# rFracList = [0.001]
# costList = [0.,0.1,0.2,0.3]
# turnoverList = [0.,0.1,0.2,0.3]

nReplicates = 1000 #xxx
tEnd = 3.65e3
dt = 1.
nProcesses = 6 #xxx
# outDir = "/scratch/strobl/data/n0Sweep_circleICs/" #xxx
outDir = "./scratch/strobl"
outName = "./paramSweep_circleICs_n0fR.csv"
# ============================= Auxillary Functions ======================================
...
def ProcessData(paramDic):
    ...
    for replicateId in tqdm(range(nReplicates),disable=True):
        ...
        for i, txName in enumerate(["MTD", "AT50"]):
            ...
            initSize = tmpDf['NCells'].iloc[0]
            initSize_R = tmpDf['NCells_R'].iloc[0]
            dataToDetermineTTP = tmpDf.tail(5)
            ttpArr[i] = dataToDetermineTTP[dataToDetermineTTP.NCells > 1.2 * initSize].Time.min()
        ...
        tmpDicList.append({"RFrac": rFrac, "InitialTumourSize": initialSize, "Cost": cost,
                           ...
                           "InitialCellNumber_Total":initSize,
                           "InitialCellNumber_R":initSize_R,
...
for initialSize, rFrac, cost, turnover, in product(initialSizeList, rFracList, costList, turnoverList):
    ...
    jobList.append({"initialSize":initialSize, "rFrac":rFrac, "turnover":turnover, "cost":cost,
                    'initialSeedingType': 'circle_fixedR', "xDim": 150, "yDim": 150,
                    'initCellNumber_R': math.ceil(rFrac*initialSize*1e4),
                    "initialSeedingDistance": int(np.sqrt(initialSize*1e4/np.pi)),
