# ====================================================================================
# Script to sweep over domain sizes to investigate how results change as we increase the
# number of cells in the system. Extracts the ttp from the CA data and computes the relative
# and absolute gains of AT. In addition, it keeps track of the competition metrics.
#  ====================================================================================
import numpy as np
import pandas as pd
...
costList = [0.]
turnoverList = [0.,0.3]
domainSizeList = [50,100,150]
nReplicates = 1000
tEnd = 3.65e3
dt = 1.
nProcesses = 6
outDir = "/scratch/strobl/data/varyDomainSize_"
outName = "./varyDomainSize.csv"
# ============================= Auxillary Functions ======================================
...
jobList = []
for initialSize, rFrac, cost, turnover, domainSize in product(initialSizeList,rFracList,costList,turnoverList,domainSizeList):
    currOutDir = os.path.join(outDir,"turnover%d/cost%d/"%(turnover*100,cost*100))
    currOutDir = os.path.join(currOutDir,"DomainSize%d/"%domainSize)
