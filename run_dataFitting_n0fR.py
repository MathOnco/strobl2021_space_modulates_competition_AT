...
patientIdList = [int(re.findall(r'\d+',x)[0]) for x in os.listdir(dataDir)][::-1]
nFits = 10
perturbICs = True
runParallel = True
nProcesses = 22
...
eps_data = 1.
optimiser_kws = {'method':'basinhopping', 'niter':50,
                 'nan_policy':'omit', 'disp':True}
solver_kws = {'nReplicates':25, 'initialSeedingType': 'random'}
params = Parameters()
params.add('cost', value=0.78, min=0, max=1, vary=False)
params.add('turnover', value=0.14, min=0, max=1, vary=False)
...
# Plot all fits
# fig, axList = plt.subplots(2, 6, sharex=True, sharey=True, figsize=(30, 15))
